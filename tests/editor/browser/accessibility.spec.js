...
async function routeSchemaRequests(page) {
  await page.route(
    'https://cdn.jsdelivr.net/npm/js-yaml@4.3.1/dist/js-yaml.min.js',
    route => route.fulfill({
      status: 200,
...
test('configuration and conditional setup controls remain accessible', async ({ page }) => {
  ...
  const results = await new AxeBuilder({ page }).include('.editor-container').withTags([
    'wcag2a',
    'wcag2aa',
    'wcag21a',
    'wcag21aa'
...
test('wizard progress retains its visual connections and fits a mobile viewport', async ({
  ...
  const mobileAxe = await new AxeBuilder({ page }).include('.editor-container').withTags([
    'wcag2a',
    'wcag2aa',
    'wcag21a',
    'wcag21aa'
...
test('validation owns, announces, navigates, and completely clears errors', async ({ page }) => {
  ...
  const validationAxe = await new AxeBuilder({ page }).include('.editor-container').withTags([
    'wcag2a',
    'wcag2aa',
    'wcag21a',
    'wcag21aa'
...
test('form and wizard expose valid DOM and platform accessibility trees', async ({
  ...
  let results = await new AxeBuilder({ page }).include('.editor-container').withTags([
    'wcag2a',
    'wcag2aa',
    'wcag21a',
    'wcag21aa'
  ...
  results = await new AxeBuilder({ page }).include('.editor-container').withTags([
    'wcag2a',
    'wcag2aa',
    'wcag21a',
    'wcag21aa'
